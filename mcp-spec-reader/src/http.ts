...
import { loadEnvConfig, createLogger, type Logger } from "@moonklabs/mcp-common";
...
// 서버 설정
const SERVER_NAME = "mcp-spec-reader";
...
function createApp(): express.Application {
  ...
  app.post("/mcp", async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ error }, "POST /mcp error");
  ...
  app.get("/mcp", async (req: Request, res: Response) => {
    ...
    // Last-Event-ID 처리 (재연결 지원) - 향후 이벤트 재전송 기능 구현 시 사용 예정
    // const lastEventId = req.headers["last-event-id"] as string | undefined;

    try {
      await session.transport.handleRequest(req, res, undefined);
    } catch (error) {
      logger.error({ error }, "GET /mcp error");
      if (!res.headersSent) {
        res.status(500).json({ error: "Internal server error" });
      }
    }
...
async function main(): Promise<void> {
  ...
  // 2. 로거 생성
  logger = createLogger(SERVER_NAME);
  ...
  const server = app.listen(PORT, HOST, () => {
    logger.info(
      ...
      `${SERVER_NAME} v${SERVER_VERSION} started`
    );
